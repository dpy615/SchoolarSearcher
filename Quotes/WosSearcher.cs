...
namespace Quotes {
    public class WosSearcher {
        ...
        private static string[] DownLoad(HttpHelper http, ref HttpResult result, string cookie, string title,bool isCn) {
            ...
            string url = item.URL;
            #region 换每页条数，目前注释
            //url = "http://apps.webofknowledge.com/Search.do?product=WOS&SID=R1yv4UYxLhASjsRWPlu&search_mode=GeneralSearch&prID=db0ae07d-c91e-4276-8a34-de033b06f90d";

            //item = new HttpItem() {
            //    URL = "http://apps.webofknowledge.com/summary.do?product=WOS&parentProduct=WOS&search_mode=GeneralSearch&qid=1&SID=" + sID + "&page=1&action=changePageSize&pageSize=50",//URL     必需项    
            //    Method = "Get",//URL     可选项 默认为Get   
            //    IsToLower = false,//得到的HTML代码是否转成小写     可选项默认转小写   
            //    Cookie = cookie,//字符串Cookie     可选项   
